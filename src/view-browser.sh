#!/usr/bin/env bash
# Build and serve the SilOS Browser target.
set -euo pipefail
...
if command -v python3 >/dev/null 2>&1 && python3 -c 'import sys; raise SystemExit(sys.version_info < (3, 0))'; then
  ...
  cat >&2 <<'EOF'
error: Python 3 is required to serve the SilOS Browser target.
Install Python 3, then rerun this script.
EOF
  exit 1
fi

echo "Serving SilOS at http://127.0.0.1:$PORT/browser-surface.html"
