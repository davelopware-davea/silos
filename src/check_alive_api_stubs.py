...
RUNTIME_BUILTINS_PATH = SOURCE_DIR / "SilOS" / "uLisp" / "BuiltinEntries.inc"
DOC_PATHS = (
...
def fail_for_name_drift(stub_text: str) -> None:
    ...
    if undocumented:
        print(
            f"Public built-ins missing from API docs and Alive stubs: {', '.join(undocumented)}",
            file=sys.stderr,
        )
    raise SystemExit(1)
...
def main() -> None:
    ...
    if current != expected:
        ...
        print(
            ...
            "'python src/check_alive_api_stubs.py --update'.",
            file=sys.stderr,
        )
        raise SystemExit(1)
    ...
    print(
        ...
        f"cover {len(implemented_names())} implemented built-ins, and match the API docs fingerprint."
    )
